...
class ViveDynamicObjectMessage(BaseModel):
    ...
    def __repr__(self):
        return f"{self.device_name} -> " \
               ...
               f"p: {round(self.p, 5)}, q: {round(self.q, 5)}, r: {round(self.r, 5)} | button: {self.button}"
...
class ViveStaticObjectMessage(BaseModel):
    ...
    def __repr__(self):
        return f"{self.device_name} -> " \
               f"x: {round(self.x, 5)}, y: {round(self.y, 5)}, z: {round(self.z, 5)} | " \
               f"qx: {round(self.qx, 5)}, qy: {round(self.qy, 5)}, qz: {round(self.qz, 5)}, qw: {round(self.qz, 5)} | button: {self.button}"

    def __str__(self):
        return self.__repr__()

# class ViveObjectList(BaseModel):
#     objects: list[]